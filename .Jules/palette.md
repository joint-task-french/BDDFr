...
## 2023-10-25 - Missing ARIA Labels on Icon-Only Actions
**Learning:** Many interactive dialogs, modal panels, and list actions in the codebase (like the `&times;` close buttons in `AttributePicker`, `GearAttributePanel`, `GeneratorForm`, and `ReviewModal`) lacked `aria-label` attributes. While some had a visual `title`, they still needed explicit `aria-label` attributes to ensure consistent screen reader support.
**Action:** When auditing or building new components, always verify that icon-only buttons (`✕`, `&times;`, emojis) include explicit and descriptive `aria-label` attributes, in French, to meet accessibility standards and enhance the overall UX.
