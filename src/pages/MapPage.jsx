...
export default function MapPage() {
    ...
    if (loadingError) return (
        <div className="flex h-full w-full items-center justify-center bg-[#0a0a0a] text-red-500 font-mono text-sm uppercase p-10 text-center leading-relaxed">
            [ERROR] Impossible de charger maps.jsonc<br/>
            Détails : {loadingError}
        </div>
    )

    if (!currentMapConfig) return (
        <div className="flex h-full w-full items-center justify-center bg-[#0a0a0a] text-gray-500 font-mono text-sm uppercase">
            [ERROR] Carte introuvable dans la configuration
        </div>
    )
    ...
    return (
        <div className="h-full w-full relative bg-[#0a0a0a] overflow-hidden z-0">
            <style>{`
                .leaflet-tooltip.tactical-map-tooltip {
                ...
                .leaflet-tooltip.tactical-map-tooltip::before,
                .leaflet-tooltip.tactical-map-tooltip::after {
            ...
            {currentMapConfig.categories?.length > 0 && (
                <div className="absolute top-4 right-4 z-[400] flex flex-col items-end">
                    <button
                        ...
                        className="mb-2 p-2 bg-tactical-panel/90 border border-tactical-border rounded shadow-lg text-gray-400 hover:text-white backdrop-blur-sm"
                        title="Filtres"
                    >
                                ...
                                {currentMapConfig.categories.map(cat => (
                                    <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                                        <input
                                            ...
                                            className="hidden"
                                        />
                                        <div className={`w-4 h-4 rounded-sm border flex items-center justify-center transition-colors shrink-0 ${activeCategories.includes(cat.id) ? 'border-shd bg-shd/20' : 'border-gray-600 bg-transparent'}`}>
                                            {activeCategories.includes(cat.id) && <div className="w-2 h-2 bg-shd rounded-sm" />}
                                        </div>
                                        <div className="w-5 h-5 flex items-center justify-center shrink-0">
                                            <GameIcon src={resolveIcon(cat.icon)} className="w-full h-full object-contain" />
                                        </div>
                ...
                >
                    ...
                    {visibleMarkers.map(marker => {
                        ...
                        const customIcon = L.divIcon({
                            html: `
                                <div style="background-color: ${catDef.color || '#3b82f6'}; box-shadow: 0 0 10px ${catDef.color || '#3b82f6'}80;" 
                                     class="w-8 h-8 rounded-full border border-black/50 flex items-center justify-center shadow-lg p-1">
                                    ${iconUrl ? `<img src="${iconUrl}" style="filter: drop-shadow(0px 2px 2px rgba(0,0,0,0.6));" class="w-full h-full object-contain" />` : ''}
                                </div>
                            `,
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    <div className="bg-tactical-panel/95 border border-tactical-border rounded p-3 backdrop-blur-sm min-w-[200px] shadow-[0_0_15px_rgba(0,0,0,0.8)] text-left font-sans">
                                        <h4 className="font-bold text-sm mb-1 uppercase tracking-widest" style={{ color: catDef.color || '#fff' }}>
                                            {marker.label}
                                        </h4>
                                        <div className="w-full h-px bg-tactical-border mb-2 opacity-50"></div>
                                        <p className="text-xs text-gray-300 m-0 leading-relaxed font-normal whitespace-pre-wrap">
                                            {marker.description}
                                        </p>
                                    </div>
                                </Tooltip>
